import os 
from ROOT import TCanvas, TMultiGraph, TGraph, gPad, TLegend
...
#        print(name)
        ...
        px.append((indicator-1)%9); py.append(float(Pos)/float(Total))
        if((indicator)%9 == 0):
            NAME.append(name)
            gr = TGraph( nn, px, py )
            GR.append(gr)
...
#        GR[i].Fit("pol4","q")
    ...
    leg.Draw()
    mg.SetTitle("Positive words propotion at NY")
