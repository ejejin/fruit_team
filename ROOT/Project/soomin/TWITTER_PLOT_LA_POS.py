...
def main():
    filename = read_file_name("LA_OUTPUT_week1n2.txt")
    can = TCanvas("can","can")
    can.SetGrid()
    mg = TMultiGraph()
    nn = 7
...
#        print(name)
        ...
        px.append((indicator-1)%7); py.append(float(Pos)/float(Total))
        if((indicator)%7 == 0):
            NAME.append(name)
            gr = TGraph( nn, px, py )
            GR.append(gr)
...
#        GR[i].Fit("pol4","q")
    ...
    leg.Draw()
    mg.SetTitle("Positive words propotion at LA(week 1&2)")
