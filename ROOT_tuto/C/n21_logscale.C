...
void n21_logscale()
{
    ...
    TPaveText *pave = new TPaveText(0.1,0.55,0.9,0.61);  // ????? difference between TPaveLabel & TPaveText ?????
   ...
   f2->Draw("same");
   f3->Draw("same");
    f1->GetYaxis()->SetMoreLogLabels(); // make more y label for log scale            //Set the kMoreLogLabels bit flag When this option is selected more labels are drawn when in log scale and there is a small number of decades (<3)
   ...
   g_2->GetYaxis()->SetNoExponent();     //  !!!! makes 10^2 ->> 100
   pad2->SetLogy();                      //  !!!!!!!! you can do this after draw 
