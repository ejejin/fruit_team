...
struct function1
{
    bool operator()(TObject *aObj)    // conversion operators
    {
...
void n2_TList_TObjString_ostringstream()
{
...
//        cout<<typeid(ss).name()<<" "<<typeid(ss.str()).name()<<" "<<typeid(ss.str().c_str()).name()<<endl;
        TObjString *s = new TObjString(ss.str().c_str());
...
//    TObjString *stest = new TObjString(ss.str());          // error!!!!!!
    TObject *Stest = (TObject*)stest;
//    cout<<"type name of Stest is : "<<typeid(Stest).name()<<endl;
    function1()(Stest);  // works in this fashion !!!!!! 
