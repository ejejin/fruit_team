...
//   h->SetCanExtend(TH1::kAllAxes);  // what is this for ??????
   h->SetStats(0);
   gRandom->SetSeed();

   for (Int_t i=0;i<15000;i++) 
   {
      Int_t rx = gRandom->Rndm()*nx; if(i%5000 == 0) cout<<rx<<"  "<<gRandom->Rndm()*nx<<endl;
      Int_t ry = gRandom->Rndm()*ny; if(i%5000 == 0) cout<<ry<<endl;
